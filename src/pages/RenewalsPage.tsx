        ...
        <div>
          ...
          {!compact && (
            <p className="muted small">
              Contracts ending in the next {RENEWAL_LOOKAHEAD_DAYS} days or expired in the last{" "}
...
export function RenewalsPage() {
  ...
  async function handleSendDigestTest() {
    ...
    try {
      ...
      const recipient = result.recipient ?? user?.email ?? "your inbox";
        ...
        <section className="content renewals-content">
          <header className="topbar">
            <div>
              ...
              <p className="muted">
                Track end dates before auto-renewals or missed notice windows. Owners and admins get email digests at
                90, 30, and 7 days out, plus on the due date.
              </p>
            </div>
          ...
          <section className="card renewals-email-card">
            ...
            {emailStatus?.configured && emailStatus.appUrlIsLocal && (
              <div className="banner renewals-email-banner renewals-email-banner--warn">
                <strong>Email links use localhost.</strong> Set <code>APP_URL=https://vendor-prototype.vercel.app</code>{" "}
            ...
            {canManageReminders && (
              <div className="renewals-email-actions">
                <button className="secondary" disabled={sendingTest} onClick={() => void handleSendTestEmail()} type="button">
                  {sendingTest
                    ? "Sending…"
                    : user?.email
              ...
              <div className="renewals-email-actions">
                ...
                >
                  {sendingDigestTest
                    ? "Sending…"
                    : user?.email
                      ? `Send weekly digest test to ${user.email}`
          ...
          {!loading && items.length === 0 && !error && <RenewalsEmptyState />}
